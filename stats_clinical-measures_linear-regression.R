...
model <- lm(MMSE_TOT ~ AGE + dwell_times_state2*LABEL, data = df_MMSEsubset)
model <- tidy(model)
write.table(model, file = "/directory/file_name.txt", sep = ",", quote = FALSE, row.names = F)
...
model_PT <- lm(PANSS_PT ~ dwell_times_state2 + AGE, data = df_PANSSsubset)
model_NT <- lm(PANSS_NT ~ dwell_times_state2 + AGE, data = df_PANSSsubset)
model_GT <- lm(PANSS_GT ~ dwell_times_state2 + AGE, data = df_PANSSsubset)
model_CS <- lm(PANSS_CS ~ dwell_times_state2 + AGE, data = df_PANSSsubset)
model_TOTAL <- lm(PANSS_TOTAL ~ dwell_times_state2 + AGE, data = df_PANSSsubset)
